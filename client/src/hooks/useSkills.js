...
export function useSkills() {
    ...
    // Add a new skill
    const addSkill = useCallback(async (name, level) => {
        const token = localStorage.getItem('auth_token');
        if (!token)
            return false;
        setLoading(true);
    ...
    const endorseSkill = useCallback(async (skillId, userId) => {
        const token = localStorage.getItem('auth_token');
        if (!token)
            return false;
        try {
            const response = await fetch(`${API_BASE}/api/skills/${skillId}/endorse`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) {
                throw new Error(`Failed to endorse skill: ${response.statusText}`);
            }
            const data = await response.json();
            if (data.success) {
                // Update local skill endorsements
                setSkills((prev) => prev.map((s) => s.id === skillId ? { ...s, endorsements: s.endorsements + 1 } : s));
                return true;
            }
        }
        catch (err) {
