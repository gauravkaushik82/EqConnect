...
function detectDocumentType(fileName: string): string {
  ...
  if (lower.includes('ugc')) return 'ugc_recognition'
  if (lower.includes('accredit')) return 'accreditation_certificate'
  if (lower.includes('degree')) return 'degree_certificate'
  return 'other'
}
...
export function useVerification() {
   ...
   */
  const submitDocuments = useCallback(async (files: File[], universityName: string) => {
    try {
      setLoading(true)
   ...
   */
  const resubmitDocuments = useCallback(async (files: File[]) => {
    try {
      setLoading(true)
   ...
   */
  const approveVerification = useCallback(async (userId: string, notes?: string) => {
    try {
      setLoading(true)
