import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { useAuth } from '../../hooks/useAuth'

function StudentDashboard() {
  const navigate = useNavigate()
  const { user, logout } = useAuth()
  const [profileComplete] = useState(75) // Example: 75% complete

  const handleLogout = () => {
    logout()
    navigate('/login')
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white">
      {/* Navbar */}
      <nav className="sticky top-0 z-50 backdrop-blur-md bg-white/80 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          <div className="text-2xl font-bold text-indigo-600">EqConnect</div>
          <div className="flex gap-4 items-center">
            <span className="text-gray-600">Welcome, {user?.full_name || 'Student'}</span>
            <button
              onClick={handleLogout}
              className="px-4 py-2 text-red-600 hover:text-red-700 font-medium border border-red-200 rounded-lg hover:bg-red-50"
            >
              Logout
            </button>
          </div>
        </div>
      </nav>

      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Welcome Section */}
        <div className="bg-white p-8 rounded-2xl shadow-lg mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">
                Welcome Back, {user?.full_name || 'Student'}! 👋
              </h1>
              <p className="text-gray-600">Ready to explore opportunities that match your skills?</p>
            </div>
            <div className="text-5xl">🎓</div>
          </div>
        </div>

        {/* Status Cards */}
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          {/* Verification Status */}
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Verification Status</h3>
              <span className="text-2xl">✓</span>
            </div>
            <p className="text-indigo-600 font-bold text-2xl mb-2">Verified</p>
            <p className="text-sm text-gray-600">Your credentials are verified and visible to recruiters</p>
          </div>

          {/* Profile Completion */}
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Profile Completion</h3>
              <span className="text-2xl">📋</span>
            </div>
            <p className="text-indigo-600 font-bold text-2xl mb-2">{profileComplete}%</p>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${profileComplete}%` }}></div>
            </div>
            <button
              onClick={() => navigate('/student/profile')}
              className="mt-3 text-sm text-indigo-600 hover:text-indigo-700 font-medium"
            >
              Complete Profile →
            </button>
          </div>

          {/* Opportunities Found */}
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Matches For You</h3>
              <span className="text-2xl">🎯</span>
            </div>
            <p className="text-indigo-600 font-bold text-2xl mb-2">12</p>
            <p className="text-sm text-gray-600">New opportunities matching your skills</p>
            <button
              onClick={() => navigate('/opportunities')}
              className="mt-3 text-sm text-indigo-600 hover:text-indigo-700 font-medium"
            >
              View Opportunities →
            </button>
          </div>
        </div>

        {/* Quick Stats */}
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          {/* Applications */}
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">📝 My Applications</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="text-gray-700">Applied</span>
                <span className="text-xl font-bold text-indigo-600">5</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="text-gray-700">Shortlisted</span>
                <span className="text-xl font-bold text-green-600">2</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="text-gray-700">Interviews</span>
                <span className="text-xl font-bold text-blue-600">1</span>
              </div>
            </div>
            <button
              onClick={() => navigate('/student/applications')}
              className="w-full mt-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium"
            >
              View All Applications
            </button>
          </div>

          {/* Upcoming Interviews */}
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">📅 Upcoming Interviews</h3>
            <div className="space-y-3">
              <div className="p-4 border-l-4 border-indigo-600 bg-indigo-50 rounded">
                <p className="font-semibold text-gray-900">Tech Company Interview</p>
                <p className="text-sm text-gray-600">April 15, 2026 at 2:00 PM</p>
              </div>
              <div className="p-4 border-l-4 border-green-600 bg-green-50 rounded">
                <p className="font-semibold text-gray-900">Product Manager Round</p>
                <p className="text-sm text-gray-600">April 18, 2026 at 3:30 PM</p>
              </div>
            </div>
            <button
              onClick={() => navigate('/messages')}
              className="w-full mt-4 py-2 border-2 border-indigo-600 text-indigo-600 rounded-lg hover:bg-indigo-50 font-medium"
            >
              Message Recruiters
            </button>
          </div>
        </div>

        {/* Action Buttons */}
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-8 rounded-2xl shadow-lg">
          <h2 className="text-2xl font-bold mb-6">Ready to Find Your Next Opportunity?</h2>
          <div className="grid md:grid-cols-3 gap-4">
            <button
              onClick={() => navigate('/opportunities')}
              className="py-3 bg-white text-indigo-600 rounded-lg hover:bg-gray-100 font-semibold transition-colors"
            >
              🔍 Browse Opportunities
            </button>
            <button
              onClick={() => navigate('/student/profile')}
              className="py-3 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 font-semibold transition-colors"
            >
              ✏️ Update Profile
            </button>
            <button
              onClick={() => navigate('/messages')}
              className="py-3 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 font-semibold transition-colors"
            >
              💬 View Messages
            </button>
          </div>
        </div>
      </div>

      {/* Footer */}
      <footer className="bg-gray-900 text-gray-400 py-8 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-sm">
          <p>&copy; 2026 EqConnect. All rights reserved.</p>
        </div>
      </footer>
    </div>
  )
}

export default StudentDashboard
